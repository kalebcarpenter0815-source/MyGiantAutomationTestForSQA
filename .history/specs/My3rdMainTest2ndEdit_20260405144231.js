//__________________________THIS IS MY THIRD TEST CASE POSITIVE USERS TEST EDIT 2!______________________________
...
// These little helper functions are here just to keep this test file easier to read
...
describe('My Login application', () => {
   ...
   it('should login with valid credentials using the standard_user username to sign in and visit Sauce Labs', async () => {
    ...
    //    await theSecurePage.addFirstItemToCartBtn.waitForClickable();
     ...
     // ================================================ 
       // THE ADDITION ONTO THE 2nd TEST IS HERE //
     // ================================================
       ...
       // Now we can continue to check to see if we can select all items into the cart all before removing them then logging out
       await theSecurePage.clickAddFirstItemToCartBtn();
       await addToCartMultipleofButtons.clickSauceLabsBikeLightAddToCartBtn();
       await addToCartMultipleofButtons.clickSauceLabsBoltTShirt();
       await addToCartMultipleofButtons.clickSauceLabsFleeceJacketAddToCartBtn();
       await addToCartMultipleofButtons.clickSauceLabsOnesieAddToCartBtn();
       await addToCartMultipleofButtons.clickSauceLabsRedTShirtAddToCartBtn();
       await addToCartMultipleofButtons.clickSauceLabsShoppingCartBtn();
       // Refresh the browser to cofirm that cart has been updated with all of the selected items and they don't delete after an accidental or purposeful refresh
       await browser.refresh();
       // Wait for the browser to load and get to the cart
       await browser.waitUntil(
        async () => (await browser.getUrl()).includes('cart.html'),
        { timeout: 10000, timeoutMsg: 'Shopping Cart page did not load after clicking on the Shopping Cart icon' }
       );
       // Continue with the test by removing all of the items
       await theSecurePage.clickRemoveItemFromCartBtn();
       await addToCartMultipleofButtons.clickRemoveSauceLabsBikeLightBtn();
       await addToCartMultipleofButtons.clickRemoveSauceLabsBoltTShirtBtn();
       await addToCartMultipleofButtons.clickRemoveSauceLabsFleeceJacketBtn();
       await addToCartMultipleofButtons.clickRemoveSauceLabsOnesieBtn();
       await addToCartMultipleofButtons.clickRemoveSauceLabsRedTShirtBtn();
       // Refresh the browser to cofirm that cart has been updated before we can go back to the main page
       await browser.refresh();
